...
SMOKER_TEMPERATURES = deque(maxlen=int(2.5 / 0.5))  # 2.5 minute window for smoker temps read every 30 seconds
FOOD_TEMPERATURES = deque(maxlen=int(10 / 0.5))    # 10 minute window over the 30 second intervals. 

SMOKER_ALERT = 15.0  # Smoker temperature decrease threshold (in degrees F)
FOOD_STALL = 1.0     # Food temperature change threshold (in degrees F)
...
def process_temperature(body, temperature_name, temperature_deque, alert_threshold):
    try:
        ...
        #print(timestamp_str) - testing
        # convert the timestamp string into a datetime object'
        timestamp_str = timestamp_str.replace('[', '')
        temperature_str = temperature_str.replace(']','')
        #print(timestamp_str) - testing to make sure brackets were removed
        #print(temperature_str) - testing for bracket removal
...
def smoker_callback(ch, method, properties, body):
    process_temperature(body, "Smoker", SMOKER_TEMPERATURES, SMOKER_ALERT)
    ch.basic_ack(delivery_tag=method.delivery_tag)

def food_a_callback(ch, method, properties, body):
    process_temperature(body, "Food A", FOOD_TEMPERATURES, FOOD_STALL)
    ch.basic_ack(delivery_tag=method.delivery_tag)

def food_b_callback(ch, method, properties, body):
    process_temperature(body, "Food B", FOOD_TEMPERATURES, FOOD_STALL)
    ch.basic_ack(delivery_tag=method.delivery_tag)
